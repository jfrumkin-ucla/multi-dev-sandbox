Feature: A User should see lists of content about a Person
  ...
  Background:
    ...
    And "sf_person" content:
      | title     | field_sf_first_name | field_sf_last_name | moderation_state |
      | John Test | John                | Test               | published        |
      | Jane Test | Jane                | Test               | published        |
      | Davis Dan | Davis               | Dan                | published        |
  ...
  Scenario: Person Directory with Exposed Filter block
    Given a block "views_exposed_filter_block:sf_person_directory-page_1" is in the "sidebar_first" region
      And I am at "/people"
    Then I should see the ".view-mode--teaser" element in the "Content" region
