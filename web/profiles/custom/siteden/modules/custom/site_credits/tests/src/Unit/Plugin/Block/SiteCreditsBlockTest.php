...
class SiteCreditsBlockTest extends UnitTestCase {
  ...
  public function testBlockForm() {
    ...
    $access_manager->checkNamedRoute("system.theme_settings_theme", ["theme" => NULL], NULL, FALSE)->willReturn(TRUE);
    $access_manager->checkNamedRoute("system.site_information_settings", [], NULL, FALSE)->willReturn(TRUE);
  ...
  public function testBlockFormSettingAccessDenied() {
    // Set up the Service Container.
    $access_manager = $this->prophesize(AccessManagerInterface::CLASS);
    $access_manager->checkNamedRoute("system.theme_settings_theme", ["theme" => NULL], NULL, FALSE)->willReturn(FALSE);
    $access_manager->checkNamedRoute("system.site_information_settings", [], NULL, FALSE)->willReturn(FALSE);
    $container = new ContainerBuilder();
    $container->set('access_manager', $access_manager->reveal());
    \Drupal::setContainer($container);
